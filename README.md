...
当前版本为 **v2.1.8**，版本号的唯一来源是 `todo_app/constants.py` 中的 `APP_VERSION`。

## 功能概览

- 创建、编辑、删除和完成待办，支持高/中/低优先级、筛选与排序。
- 为任务设置截止时间和提前提醒；到期任务集中显示在一个软件内提醒窗口，可逐项完成、默认推迟 1 小时或通过箭头选择其他时长，也可忽略。
- “忽略”会清除任务的时间约束但保留任务和提醒偏好，不会删除任务或将其标记为完成。
- 提醒窗口首次显示时自动定位，拖动后在本批次刷新和隐藏恢复时保留位置，超出屏幕可用区域时才纠正。新增或编辑任务期间暂时隐藏提醒，继续收集到期任务；保存或取消后恢复同一提醒批次。
- 系统托盘支持显示/隐藏窗口、快速添加和退出；最小化或关闭到托盘时不发送系统气泡。
- 自动跟随系统深浅色主题，使用内置 HarmonyOS Sans SC 字体并在资源不可用时安全回退。
- 适配 320px 最小窗口宽度；任务卡片最多预览 3 行，隐藏后续内容时在末行显示省略号，原始换行与全文保留，可通过悬停浮层完整查看。
- 鼠标滚轮按字体行高平滑滚动，触控板像素输入直接跟随；拖动滚动条、键盘定位或筛选更新会接管滚动位置。
- 秒级计时刷新只处理可见卡片，滚动、调整窗口或从托盘恢复时立即更新新显示的倒计时；到期提醒仍检查全部任务。
- 待办数据使用原子替换和单份有效备份，损坏主文件不会被静默覆盖。

## v2.x 近期变化

- **v2.1.8**：屏幕外卡片与隐藏/最小化窗口跳过普通 Tick 的计时呈现，卡片重新可见时立即刷新；补充 100 次编辑窗口开关的控件及主题连接释放验证。
- **v2.1.7**：多行任务限制为 3 行预览，编辑/删除按钮紧凑居中；滚轮改为固定步长的短缓动，避免高卡片导致大幅跳动，保留触控板和直接定位响应。
- **v2.1.6**：任务增删改、完成切换与提醒处置改用差量列表协调，复用未变化卡片并保持筛选、排序和滚动位置；加载时修复规范化后重复的 ID，保证旧数据可以启动并独立处理每项任务。
- **v2.1.5**：提醒内容更新不再重置拖动位置；新增/编辑任务期间提醒让出焦点，结束后恢复，关闭的编辑窗口及时释放；提醒卡片间隙跟随深浅主题。
- **v2.1.3**：空闲秒级刷新只在卡片计时或完成状态实际变化时更新界面，避免重复图标、样式与列表布局工作。
- **v2.1.2**：通知行改用分裂式“推迟1h”按钮，主区域一键推迟，箭头保留其他时长选项。
...
- 加载时按文件顺序规范化任务 ID；缺失、非法或与已加载任务重复的 ID 会重新生成，保留任务内容。读取不会回写文件，修复后的 ID 在下一次正常保存时持久化。
...
- `main` 的最新构建会按 `APP_VERSION` 更新可移动的预发布标签，例如 `2.1.8` 对应 `pre2.1.8`，并更新不作为 Latest 的 Pre-release。
