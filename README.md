...
当前版本为 **v2.1.4**，版本号的唯一来源是 `todo_app/constants.py` 中的 `APP_VERSION`。
...
- **v2.1.4**：任务增删改、完成切换与提醒处置改用差量列表协调，复用未变化卡片并保持筛选、排序和滚动位置。
...
- `main` 的最新构建会按 `APP_VERSION` 更新可移动的预发布标签，例如 `2.1.4` 对应 `pre2.1.4`，并更新不作为 Latest 的 Pre-release。
