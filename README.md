...
当前版本为 **v2.1.2**，版本号的唯一来源是 `todo_app/constants.py` 中的 `APP_VERSION`。
...
- **v2.1.2**：通知行改用分裂式“推迟 1 小时”按钮，主区域一键推迟，箭头保留其他时长选项。
...
- `main` 的最新构建会按 `APP_VERSION` 更新可移动的预发布标签，例如 `2.1.2` 对应 `pre2.1.2`，并更新不作为 Latest 的 Pre-release。
