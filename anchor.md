...
- 列表交互：
  ...
  - 卡片宽度始终服从列表视口，任务文字区域与计时区域缩小时分别至少保留 150px 与 50px；编辑/删除按钮作为计时区域上方的悬停浮层显示，不参与正文与计时区域的宽度分配，显示或隐藏时不得重排内容；任务文字按标签实际像素宽度省略，计时文字保留完整内部文本并从末尾向前省略，省略时通过 tooltip 提供全文。列表项不提供选择态，避免绘制与卡片几何不一致的选中边框。
