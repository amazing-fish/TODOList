...
- 提醒流程：`master_timer` 每秒触发 `tick_update` 扫描完整 `self.todos`，提醒不受当前列表筛选影响，可见卡片循环只刷新倒计时；一轮请求先写入提醒去重字段，再汇总到任意时刻唯一的非模态软件内 `NotificationDialog`，同一任务按 ID 去重且“已到期”覆盖“提前提醒”。同批任务只播放一次 `play_sound_effect` 软件提醒音，窗口打开期间的新批次追加到原窗口，不创建 Windows 系统任务通知、Toast 或任务到期托盘气泡。提醒唤醒时优先调用原生接口恢复并前置主窗口，若平台不支持则临时添加 `WindowStaysOnTopHint` 保障可见，之后自动回退。批量完成只处理勾选任务；每行的推迟入口直接处理当前任务并可独立选择时长，两类操作均各执行一次存储与列表刷新。主窗口隐藏到托盘时同步隐藏提醒窗口但保留批次，恢复主窗口时重新显示同一批次，明确忽略或真正退出后才释放 Qt 对象与主题信号连接。
...
## 最近约定变更
- 2026-07-17：feature，提醒窗口改为每个任务行内独立选择推迟时间，推迟不再依赖勾选状态，批量勾选仅用于完成任务，版本更新至 `v1.8.0`。
