...
- 字段约定：
  ...
  - `text`（str）任务内容，列表使用纯文本保留 `LF` / `CRLF` 换行，长单行按可用宽度自动换行；显示层不得修改存储原文。
...
- 列表交互：
  ...
  - 卡片宽度始终服从列表视口，任务文字区域与计时区域缩小时分别至少保留 150px 与 50px；编辑/删除按钮作为计时区域上方的悬停浮层显示，不参与正文与计时区域的宽度分配，显示或隐藏时不得重排内容。任务正文以纯文本保留原始 `LF` / `CRLF`，长中文、英文和连续字符使用 `WrapAtWordBoundaryOrAnywhere` 在卡片内正常换行，不再强制生成单行省略摘要；viewport 每次变化后先稳定卡片宽度，再用真实正文宽度同步卡片与 `QListWidgetItem` 高度，重新拉宽时高度必须收缩。计时文字保留完整内部文本并从末尾向前省略，省略时通过 tooltip 提供全文。列表项不提供选择态，避免绘制与卡片几何不一致的选中边框。
