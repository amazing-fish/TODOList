...
- 截止时间编辑：新增任务的默认截止时间严格取本地当前时间一小时后，日期与时间来自同一目标时刻并按可见分钟保存；时间使用支持滚轮和上下键微调的 `QTimeEdit`，日期使用低频内联 `QDateEdit` 日历下拉。选择日期直接应用，不再创建独立日期确认窗口。编辑已有任务时，未改日期与分钟则保留原截止时间的完整精度，实际调整后秒与毫秒归零。
