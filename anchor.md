...
- **运行**：开发环境可执行 `python main.py`；GitHub Actions 工作流 `build-exe.yml` 在手动触发、推送到 `main` 或推送 `v*` 标签时生成 Windows 单文件可执行程序并上传保留 7 天的临时 Artifact。只有推送 `v*` 标签才会基于该固定标签提交创建或更新长期保留的正式 Release；`main` 与手动构建不会创建或移动标签，也不会修改 Release。
...
## 最近约定变更
- 2026-07-15：refactor，Windows 打包流程分离正式 Release 与临时 Artifact；仅推送 `v*` 标签发布，`main` 与手动构建只上传保留 7 天的 Artifact，不再创建或移动标签（不触发版本号）。
